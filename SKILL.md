...
Select the best frame from Slide 1 as the video's cover/thumbnail image. This is what users see in the video feed before clicking.

**Selection strategy:** Use the frame where the most hook elements are visible — typically Slide 1, Frame 2 (at 0.3s, when the hero element has appeared but animations are still dynamic). This frame has:
- Hook title visible and settled
- Visual hero (number/icon) present
- Background atmosphere established
- Still has visual energy (not fully static)

The cover image is auto-extracted by `html2video.py` as `{output_name}_cover.png`, or overridden with `--cover-frame` to pick a specific frame index.
